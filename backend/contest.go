...
func SubmitC(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != "POST" {
		if session.Values["isLogin"] == true {
			if model.IsAccVerifed(r) {
				path := r.URL.Path
				contestIDSerialIndex := strings.TrimPrefix(path, "/submitC/")

				need := "-"
				index := strings.Index(contestIDSerialIndex, need)
				var OJ, pNum string
				...
				contestIDStr := contestIDSerialIndex[0:index]
				contestID, _ := strconv.Atoi(contestIDStr)
				serialIndex := contestIDSerialIndex[index+1:]

				//getting OJ & pNum for this contest & serial number
				//connecting to DB
				DB, ctx, cancel := db.Connect()
				defer cancel()
				defer DB.Client().Disconnect(ctx)
				...
				for i := 0; i < len(dbQuery.ProblemSet); i++ {
					if dbQuery.ProblemSet[i].SerialIndex == serialIndex {
						OJ = dbQuery.ProblemSet[i].OJ
						pNum = dbQuery.ProblemSet[i].PNum
					}
				}
