...
$("select[name=usernameDF]").change(function () {
...
$("input[name=pNumDF]").change(function () {
    $('#loadingGifPNum').css("display", "inline-block");
    displayFilter();
})

function displayFilter() {
    let username = $("input[name=usernameDF]").val().trim();
    let pNum = $("input[name=pNumDF]").val();
    username = username.toLowerCase();

    //selecting submission according to requirement
    cSubmissionList = [];

    if (username != "" && pNum != "All") {
        for (i = 0; i < tempList.length; i++) {
            listUsername = tempList[i].Username.toLowerCase();

            if (username == listUsername && pNum == tempList[i].SerialIndex) {
                cSubmissionList.push(tempList[i]);
            }
        }
    } else if (username != "" && pNum == "All") {
        for (i = 0; i < tempList.length; i++) {
            listUsername = tempList[i].Username.toLowerCase();

            if (username == tempList[i].Username) {
                cSubmissionList.push(tempList[i]);
            }
        }
    } else if (username == "" && pNum != "All") {
        for (i = 0; i < tempList.length; i++) {
            if (pNum == tempList[i].SerialIndex) {
                cSubmissionList.push(tempList[i]);
            }
        }
    } else if (username == "" && pNum == "All") {
        cSubmissionList = tempList;
    }
